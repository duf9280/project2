...
@WebServlet("/market/view_lecture")
public class Market_view_lecture extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    public Market_view_lecture() {
        super();
    }
...
//			System.out.println(code);
			...
			if(search_view !=null && !search_view.equals("")) {	//검색	
				count_v = dao.getAllCount_com(search_view,field_view,code,uid);//총 리뷰 수 설정
			}else {
				count_v = dao.getAllCount1_com(code,uid);//총 리뷰 수 설정 > 검색이 아닐 때
			}
			...
			if(search_view != null && !search_view.equals("")) {//검색일때
				rev= dao.allReview1(startNum,endNum,search_view,field_view,code,uid);
			}else {
				rev= dao.allReview(startNum,endNum,code,uid);
			}
...
//			System.out.println(field_view);
			...
			if(!cat.equals("2")) {
				RequestDispatcher dis = request.getRequestDispatcher("/market/view_lecture.jsp?code="+code+"&category="+category+"&uid="+uid+"&tb_id="+id+"&pageNum="+pageNum+"&field_view="+field_view+"&search_view="+search_view+"");
				dis.forward(request, response);	
			}else {
				RequestDispatcher dis = request.getRequestDispatcher("/manage/bbs/view_lecture.jsp?cat="+cat+"&code="+code+"&category="+category+"&uid="+uid+"&tb_id="+id+"&pageNum="+pageNum+"&field_view="+field_view+"&search_view="+search_view+"");
				dis.forward(request, response);	
			}
		}catch(Exception e) {
			e.printStackTrace();
