			...
			<c:forEach var="m" items="${v }">	
				<div style="text-align:center; width:25%;">
					<div style="display:flex; flex-direction: column;height:200px; width:200px; align-items: center;">
						<a href="view_${param.code}?cat=1&code=<%=code%>&category=${param.category}&uid=${m.uid }&id=${m.id }&g=${param.g}&tb_id=${sessionScope.id}&pageNum_view=1&pageNum=${pageNum}&field_view=${field_view}&search_view=${search_view}">						
					...
					<div>
						<a href="view_${param.code}?code=<%=code%>&category=${param.category}&uid=${m.uid }&id=${m.id }&g=${param.g}&tb_id=${sessionScope.id}&pageNum_view=1&pageNum=${pageNum}&field_view=${field_view}&search_view=${search_view}">					
							${m.subject}<br>
							<c:if test="${param.code ne 'product' }">
...
<!-- 			목록, 글쓰기버튼 -->
			<div><a href="list?code=<%=code%>&category=${param.category}&pageNum=${pageNum}">[목록]</a></div>
			
			<c:choose>
				<c:when test="${param.code eq 'lecture'}">
					<div><a href="write_lecture.jsp?code=<%=code%>&category=${param.category}&pageNum=${pageNum}">[강의등록]</a></div>
				</c:when>
				<c:when test="${param.code eq 'product'}">
					<div><a href="write_product.jsp?code=<%=code%>&category=${param.category}&pageNum=${pageNum}">[강의등록]</a></div>
				</c:when>
				<c:otherwise>
				</c:otherwise>
			</c:choose>
...
<div style="width:100%;">
	<div align=center>
		...
		<c:if test="${startPage > pageBlock }">
			<a href="list?code=<%=code %>&pageNum=${startPage - pageBlock }&category=${param.category}">[이전] </a>
		</c:if>	
		...
		<c:forEach var="i" begin="${startPage }" end="${endPage }">
			<c:choose>
				<c:when test="${pageNum== i} ">					
					<a href="list?code=<%=code %>&pageNum=${i }&category=${param.category}" style="font-weight:bold; color:red;">[${i }]</a>					
				</c:when>
				<c:otherwise>
					<a href="list?code=<%=code %>&pageNum=${i }&category=${param.category}">[${i }] </a>
				</c:otherwise>
			</c:choose>
		</c:forEach>	
		<!-- 다음 링크 -->
		<c:if test="${endPage < pageCount }">
			<a href="list?code=<%=code %>&pageNum=${startPage + pageBlock }&category=${param.category}">[다음] </a>
		</c:if>
		</c:if>
